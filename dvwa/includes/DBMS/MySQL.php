...
error_log(mysqli_query($conn, $drop_db));
if( !@mysqli_query($conn, $drop_db ) ) {
	dvwaMessagePush( "Could not drop existing database<br />SQL: ".mysqli_error( $conn ) );
	dvwaPageReload();
}

$create_db = "CREATE DATABASE {$_DVWA[ 'db_database' ]};";
if( !@mysqli_query($conn, $create_db ) ) {
	dvwaMessagePush( "Could not create database<br />SQL: ".mysqli_error($conn) );
	dvwaPageReload();
}
...
// Create table 'users'
if( !@mysqli_select_db($conn, $_DVWA[ 'db_database' ] ) ) {
...
if( !mysqli_query($conn, $create_tb ) ) {
	dvwaMessagePush( "Table could not be created<br />SQL: ".mysqli_error($conn) );
	dvwaPageReload();
}
...
$baseUrl  = 'http://'.$_SERVER[ 'SERVER_ADDR' ].":".$_SERVER[ 'SERVER_PORT' ].$_SERVER[ 'PHP_SELF' ];
error_log($baseUrl."-----");
...
if( !mysqli_query($conn, $insert ) ) {
	dvwaMessagePush( "Data could not be inserted into 'users' table<br />SQL: ".mysqli_error($conn) );
	dvwaPageReload();
}
...
if( !mysqli_query($conn,$create_tb_guestbook ) ) {
	dvwaMessagePush( "Table could not be created<br />SQL: ".mysqli_error($conn) );
	dvwaPageReload();
}
...
if( !mysqli_query($conn,$insert ) ) {
	dvwaMessagePush( "Data could not be inserted into 'guestbook' table<br />SQL: ".mysqli_error($conn) );
	dvwaPageReload();
}
